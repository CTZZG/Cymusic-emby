...
const setTrackSource = async (track: Track, autoPlay = true) => {
	...
	// 在 setTrackSource 函数的最后添加
	if (autoPlay && (track.platform === 'emby' || track._source === 'emby')) {
    // 上报播放开始
    	await reportEmbyPlaybackProgress(track as IMusic.IMusicItem, 0, false)
	}
}
/**
...
const getEmbyPlayUrl = async (musicItem: IMusic.IMusicItem): Promise<string | null> => {
    try {
        ...
        if (!tokenInfo) {
            logError('Emby token not available')
            return null
        }

        const config = getEmbyConfig()
        if (!config) {
            logError('Emby config not available')
            return null
        }
 ...
 * 上报播放进度到Emby
 * @param musicItem 音乐项目
 * @param positionTicks 播放位置（以ticks为单位）
 * @param isPaused 是否暂停
 */
const reportEmbyPlaybackProgress = async (
    ...
    try {
        ...
        logInfo('Reported playback progress to Emby')

    } catch (error) {
        logError('Failed to report playback progress to Emby:', error)
    }
}
...
if (!source) {
    ...
    if (musicItem.platform === 'emby' || musicItem._source === 'emby') {
        ...
        if (embyUrl) {
            ...
            logInfo('成功获取Emby播放URL:', embyUrl)
        } else {
            logError('获取Emby播放URL失败')
